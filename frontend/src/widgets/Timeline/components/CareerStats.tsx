...
export const CareerStats: React.FC<CareerStatsProps> = ({ companies }) => {
    ...
    const { experience, companiesCount, positionsCount, competencies } =
        useMemo(() => {
            ...
            const comps = [
                ...new Set(
                    allPositions.flatMap((p) => p.tags.map((tg) => tg[lang])),
                ),
            ];
    ...
    const metrics = [
        ...
        },
        {
            ...
            label:
                companiesCount === 1
                    ? t("career.stats.company")
                    : t("career.stats.companies"),
    ...
    return (
        ...
        >
            {/* Status profissional */}
            <div
                className="career-aside__card career-aside__status"
                style={{ ["--card-i" as string]: 0 }}
            >
                ...
                <div className="career-aside__level">
                    <span className="career-aside__level-label">
                        {t("career.aside.level")}
                    </span>
                    <span className="career-aside__level-value">
                        {t("career.aside.seniority")}
                    </span>
                </div>
            </div>
            ...
            {/* Métricas */}
            <div
                className="career-aside__card"
                style={{ ["--card-i" as string]: 1 }}
            >
                <h3 className="career-aside__title">
                    {t("career.aside.overview")}
                </h3>
                <ul className="career-aside__metrics">
                    {metrics.map((m, i) => (
                        ...
                        >
                            <span
                                className="career-aside__metric-icon"
                                aria-hidden="true"
                            >
                                {m.icon}
                            </span>
                            <span className="career-aside__metric-label">
                                {m.label}
                            </span>
                            <span className="career-aside__metric-value">
                                {m.value}
                            </span>
                        </li>
                    ))}
